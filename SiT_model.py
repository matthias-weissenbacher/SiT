...
class Sym_Break_Linear_Block(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_features, num_patches, num_heads):
        ...
        self.attn2 = Attention_Basic(in_features // 2, num_patches=num_patches, num_heads=num_heads // 2, qkv_bias=False)
    ...
    def forward(self, x):
        ...
        W0 =  torch.index_select(self.weights0 , 1, self.idxsW0.flatten().to(x.device)).reshape((self.in_features//4,1,self.kernel_size0,self.kernel_size0))
        W =  torch.index_select(self.weights , 1, self.idxsW.flatten().to(x.device)).reshape((self.in_features//4,1,self.kernel_size,self.kernel_size))
        W1 =  torch.index_select(self.weights1 , 1, self.idxsW.flatten().to(x.device)).reshape((self.in_features//4,1,self.kernel_size,self.kernel_size)) 
        Wa =  torch.index_select(self.weightsa , 1, self.idxsW.flatten().to(x.device)).reshape((self.in_features//2,1,self.kernel_size,self.kernel_size))
        W1a =  torch.index_select(self.weights1a , 1, self.idxsW.flatten().to(x.device)).reshape((self.in_features//2,1,self.kernel_size,self.kernel_size))
        W2 =  torch.index_select(self.weights2 , 1, self.idxsW2.flatten().to(x.device)).reshape((1,self.in_features//2,(self.patch_size//2)**2)).permute(0,2,1)
        ...
        qkv = self.norm(self.get_patches_flat(torch.cat([q,k,self.projx(x.permute(0,2,3,1)),],dim=-1),ps=self.patch_size//2))
        f = 2*f
        ...
        qkv = self.norm2(self.get_patches_flat(torch.cat([q,k,self.projx2(x.permute(0,2,3,1)),],dim=-1),ps=self.patch_size//2))
        q, k, v = qkv[:,:,:f], qkv[:,:,f:2*f], qkv[:,:,2*f:]
...
class VisionTransformer(nn.Module):
	...
	def forward(self, x):
		...
		y = torch.cat([ self.projf(nn.GELU()(y )),self.projf2(nn.GELU()(x[:, 0:1])).reshape(-1,4,4)],dim=-1).flatten(1)    
		return y #x[:, 0]+
...
class SietTiny(nn.Module):
    def __init__(self, img_size=64, patch_size=8,  patch_size_local=8, in_chans=3, embed_dim=32, depth=1,
        ...
        self.graph_block = Sym_Break_Linear_Block(img_size=img_size_c, patch_size = patch_size,in_features =embed_dim,num_patches =self.num_patches, num_heads=num_heads)
